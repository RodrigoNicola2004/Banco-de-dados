...
struct Transaction {
    int id;
    time_t timestamp; // marcaçao tem p/ Wait-Die
    bool active;     // true -> ativo, false -> desativado (precisa reiniciar)

    Transaction(int _id) {  //inicializa
        id = _id;
        timestamp = time(NULL);
        active = true;
    }

    void restart() {    //reinicia --> novo timestamp
...
bool detect_deadlock(Transaction* t, Resource* r) { //detectar deadlock
    for (Transaction* waiting_t : all_transactions) {
        // Verifica se 't' espera por 'r', que é de 'waiting_t' (inativa/esperando)
        if (waiting_t->id != t->id && !waiting_t->active) {
            if (r->locked_by_tid == waiting_t->id) {
                cout << "DEADLOCK DETECTADO entre T" << t->id << " e T" << waiting_t->id << "!" << endl;
                return true;
            }
        }
    }
...
bool acquire_lock(Transaction* t, Resource* r) {
    DWORD wait_result;
    bool acquired = false;
    ...
    if (r->locked_by_tid == -1) {//rec livre: t ganha bloqueio
        ...
        for (Transaction* current_t : all_transactions) {   // encontra quem tem o bloqueio
        ...
        if (t->timestamp < holder_timestamp) {
            ...
            cout << "T" << t->id << " (Timestamp: " << t->timestamp << ") e mais jovem que T" << holder_t->id << " (Timestamp: " << holder_timestamp << ") e morre para " << r->item_id << " (Wait-Die)" << endl;
            t->active = false;
            acquired = false;
        }
    }
...
void release_lock(Transaction* t, Resource* r) {
...
DWORD WINAPI run_transaction(LPVOID param) {
    ...
    while (t->active) {
        ...
        if (!acquire_lock(t, X)) {  //Tenta bloquear X
            if (!t->active) {
                Sleep(rand() % 200 + 100);
                continue;
            }
        ...
        if (!acquire_lock(t, Y)) {  // - - Y , se der ruim libera x e tenta dnv
            release_lock(t, X); //libera x p/ n travar os outros 
            if (!t->active) {
                Sleep(rand() % 200 + 100);
                continue;
            }
...
int main() {
    ...
    for (int i = 0; i < NUM_TRANSACTIONS; i++) {
        ...
        Sleep(rand() % 150 + 50); // Pequena pausa para variar os timestamps
    }
